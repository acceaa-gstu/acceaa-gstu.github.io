...
-- ============================================================================
-- PERSISTENT VISITOR IDENTITY (U0001, U0002, ...)
-- ============================================================================
-- One row per returning DEVICE/BROWSER, not per visit. The site stores a
-- random id in that browser's localStorage (persists across visits, until
-- the visitor clears site data - a bigger step than the tab-only session_id
-- above) and looks it up here on every visit, so the same person coming
-- back later gets the same row - and the same "U0001"-style number, which
-- is just this table's own sequential id, zero-padded for display.
...
-- No anonymous INSERT/UPDATE policy needed - this is the only way in or out
-- for a visitor's browser, and it only ever touches that one device_id's
-- own row (find it, touch its last_seen_at; otherwise create it).
...
-- ============================================================================
-- MEMBER CONTACT-LINK CLICKS
-- ============================================================================
-- Which member's LinkedIn/Facebook/WhatsApp/Email a visitor clicked, from
-- the directory card. Tells staff which members' contact info actually
-- gets used - not who clicked it in real life, only which anonymous
-- visitor (U-code) did.
